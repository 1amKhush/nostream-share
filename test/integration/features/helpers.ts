...
secp256k1.utils.sha256Sync = (...messages: Uint8Array[]) =>
  Uint8Array.from(
    messages
      .reduce(
        (hash: Hash, message: Uint8Array) => hash.update(message),
        createHash('sha256')
      )
      .digest()
  )
...
export async function createEvent(
  ...
  const event: Event = {
    ...
    kind: input.kind,
    created_at:
      input.created_at ?? Math.floor(Date.now() / 1000) + eventCount++,
    content: input.content ?? '',
    tags: input.tags ?? [],
  } as any

  const id = createHash('sha256')
    .update(JSON.stringify(serializeEvent(event)), 'utf8')
    .digest('hex')

  const sig = Buffer.from(secp256k1.schnorr.signSync(id, privkey)).toString(
    'hex'
  )
